...
const useDisputesQuery = (marketAddress: string, networkId: NetworkId, options?: UseQueryOptions<Disputes>) => {
    return useQuery<Disputes>(
        ...
        async () => {
            ...
            const [disputes, isMarketClosedForDisputes, allOpenDisputesCancelledToIndexForMarket] = await Promise.all([
            ...
            const mappedDisputes = disputes.map((dispute: DisputeInfo) => {
                const isOpenDisputeCancelled =
                    (isMarketClosedForDisputes || dispute.disputeNumber <= allOpenDisputesCancelledToIndexForMarket) &&
